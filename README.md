...
- `scripts/parity/` the check. Pulls the component set from the REST API, compares properties and options with the code contract, compares bound colours with the token export, prints a report, exits 1 on drift. The last in sync run is in [docs/parity-report-in-sync.md](docs/parity-report-in-sync.md).
