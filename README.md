...
- `scripts/parity/` the check. Pulls the component set from the REST API, compares properties and options with the code contract, compares bound colours with the token export, prints a report, exits 1 on drift. The last in sync run is in [docs/parity-report-in-sync.md](docs/parity-report-in-sync.md). [docs/parity-report-new-prop.md](docs/parity-report-new-prop.md) is the report from the moment a Selected label property was added in Figma before the code had it.
