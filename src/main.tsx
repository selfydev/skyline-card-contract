...
const ar = { title: 'دخول الصالة', description: 'ثلاث ساعات في صالة سكاي لاين قبل المغادرة.', price: '110 د.إ', originalPrice: '137 د.إ', badgeLabel: 'خصم 20%' };
...
function Matrix() {
  ...
  for (const direction of ADD_ON_CARD_PROPS.direction) for (const state of ADD_ON_CARD_PROPS.state) for (const discount of [false, true]) {
    ...
    const cta = direction === 'rtl' ? 'إضافة' : 'Add';
